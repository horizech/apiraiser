...
const CreateEditTableRecord = ({
    ...
    const handleOnSubmit = (data, event) => {
        ...
        tableColumns
            .filter((column) => column.Datatype == "Integer")
            .map((column) => {
        ...
        if (mode === "create") {
            dispatch(tablesActions.insertRow(schema, table, data));
        } else if (mode === "edit") {
            dispatch(tablesActions.updateRow(schema, table, data));
        }
    };
        ...
        <React.Fragment>
            {!tableColumns && (
                <div className="row">
                    ...
                    >
                        <p>Table columns were not found!</p>
                    </div>
                    <div className="col-sm-1 col-md-3"></div>
                </div>
            )}
                    ...
                    <Form
                        ...
                        render={({
                            ...
                            <form
                                name="form"
                                className="row"
                                id="createEditTableForm"
                                onSubmit={(e) => handleOnSubmit(values, e)}
                            >
                                {tableColumns
                                    .filter(
                                        (column) =>
                                            !getIsColumnReadOnly(column.Name) ||
                                            mode == "edit"
                                    )
                                    .map((column) => (
                                        <Field
                                            ...
                                            type={GetElementTypeFromDataType(
                                                column.Datatype
                                            )}
                                            key={column.Name}
                                            name={column.Name}
...
function mapStateToProps(state) {
    ...
    const { insertedRowSuccess, insertingRow, updatedRowSuccess, updatingRow } =
...
const connectedCreateEditTableRecord = connect(mapStateToProps)(
    CreateEditTableRecord
);
