...
import { Schemas, sidebarSubMenus } from "../constants";
...
const Sidebar = ({
    ...
    const [subMenuActive, setSubMenuActive] = React.useState(
        params.subMenuActive
    );
    const [menuItemActive, setMenuItemActive] = React.useState(
        params.menuItemActive
    );
    const [toggled, setToggled] = React.useState();

    React.useEffect(() => {
        if (loggedIn && tableslist === null) {
            getTables(Schemas.Data);
        }
    });
    ...
    const renderTables = (tables) => {
        return (
            <SubMenu
                onOpenChange={(open) => {
                    setSubMenu(open, "table");
                }}
                open={subMenuActive === "table"}
                title="Tables"
            >
                <Link to="/table/create" className="btn btn-link">
                    Create New Table
                </Link>
                {tables &&
                    tables.map((table) => (
                        ...
                        >
                            ...
                            >
                                <span onClick={() => gotoTableDataPage(table)}>
                                    {table}
                                </span>
                                <ButtonIcon
                                    className="ml-3"
                                    icon="table"
                                    color="#fffc"
                                    onClick={() => gotoTableDesignPage(table)}
                                />
                            </div>
                        </MenuItem>
                    ))}
        ...
        <React.Fragment>
            {loggedIn && (
                <ProSidebar key={"prosidebar"} toggled={toggled}>
                    <SidebarHeader>
                        <div className="sidebar-header">Apiraiser</div>
                    </SidebarHeader>
                    <SidebarContent>
                        <Menu iconShape="square">
                            <MenuItem>Apiraiser</MenuItem>
                            {/* {renderMenu()} */}
                            <MenuItem onClick={() => gotoPath("/user")}>
                                Current User
                            </MenuItem>
                            <MenuItem onClick={() => gotoPath("/users")}>
                                Users
                            </MenuItem>
                            <MenuItem onClick={() => gotoPath("/settings")}>
                                Settings
                            </MenuItem>
                            <MenuItem onClick={() => gotoPath("/roles")}>
                                Roles
                            </MenuItem>
                            ...
                            <SubMenu
                                onOpenChange={(open) => {
                                    setSubMenu(open, "Components");
                                }}
                                open={subMenuActive === "Components"}
                                title="Components"
                            >
