...
const AddTableColumn = ({
  ...
  const prepareFormData = (data) => {
    let result = {
      ...
      ForeignSchema:
        data.IsForeignKey && data.ForeignSchema
          ? data.ForeignSchema.value
          : null,
      ForeignTable:
        data.IsForeignKey && data.ForeignTable ? data.ForeignTable.value : null,
      ForeignName:
        data.IsForeignKey && data.ForeignName ? data.ForeignName.value : null,
  ...
  const handleValidate = (values) => {
    ...
    if (
      values.ForeignSchema != null &&
      values.ForeignSchema.value != curForeignSchema
    ) {
    ...
    if (
      values.ForeignTable != null &&
      values.ForeignTable.value != curForeignTable
    ) {
  ...
  const loadForeignTableOptions = (values) => {
    ...
    if (foreignSchema) {
      tablesService.getTables(foreignSchema).then((result) => {
        if (result.Success) {
          console.log(result.Data);
          foreignTableOptions = result.Data.map((x) => {
  ...
  const loadForeignNameOptions = (values) => {
    ...
    if (foreignSchema && foreignTable) {
      tablesService
        .getTableColumns(foreignSchema, foreignTable)
        .then((result) => {
          if (result.Success) {
            foreignNameOptions = result.Data.map((x) => {
  ...
  const getColumns = (values) => {
    ...
    columns.forEach((column, i) => {
      switch (column.Name) {
        ...
        case "DefaultValue":
          ...
          const dataTypeSelectedValue = values["Datatype"]
    ...
    return filteredColumns.map((column) => (
      <Field
        ...
        name={column.Name}
        required={true}
        component={DynamicElementAdapter}
        column={column}
      />
    ));
  };
  ...
  if (addingColumn) {
    return (
      <div className="row">
        ...
        <div
          align="center"
          className="col-sm-10 col-md-6 col-md-offset-3"
      ...
      <div>
        <Form
          ...
          render={({ handleSubmit, form, submitting, pristine, values }) => (
            <React.Fragment>
              {console.log(
                clearForeignTableOnNextRender,
                clearForeignNameOnNextRender
              )}
              {clearForeignTableOnNextRender &&
                form.change("ForeignTable", undefined)}
              {clearForeignNameOnNextRender &&
                form.change("ForeignName", undefined)}
              <form
                name="form"
                className="row"
                id="addColumnForm"
                onSubmit={(e) => handleOnSubmit(values, e)}
              >
                {getColumns(values)}

                <br />
                {showJson && (
                  <p>{JSON.stringify(prepareFormData(values), 0, 2)}</p>
                )}
