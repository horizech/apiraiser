...
const CreateEditModal = ({
  ...
  const [isJsonVisible, setJsonVisible] = React.useState(false);
  ...
  const handleOnSubmit = (values) => {
    columns
      .filter((column) => column.IsSelectList)
      .map((column) => {
        values[column.Name] = values[column.Name].value;
      });
    columns
      .filter((column) => column.needParse)
      .map((column) => {
        values[column.Name] = parseInt(values[column.Name]);
      });
    columns
      .filter(
        (column) => column.Datatype == "Boolean" && values[column.Name] == null
      )
      .map((column) => {
    ...
    //   .map((column) => {
    //     values[column.Name] = Object.keys(values[column.Name]).map((x) =>
    //       parseInt(x)
    //     );
    //   });
  ...
  const getIsSelectColumn = (column) => {
    ...
    if (selectColumns && selectColumns.length) {
      const selectColumnSearch = selectColumns.filter(
                  ...
                  >
                    {columns &&
                      columns
                        .filter(
                          (column) =>
                            !getIsColumnReadOnly(column) || mode == "edit"
                        )
                        .map((column) => (
                          <Field
                            isWorking={false}
                            ...
                            selectOptions={
                              selectOptions ? selectOptions[column.Name] : null
                            }
                            isSelect={
                              isSelectList ? isSelectList[column.Name] : false
                            }
                            column={column}
                          />
                        ))}
