...
import { userService, rolesService, tablesService } from "../services";
import { DialogModal } from "../modals";
import { ApiraiserTable } from "../components";
...
const UsersPage = ({ match, location, dispatch, user, loggedIn }) => {
    ...
    const [loadingSettingColumns, setLoadingSettingColumns] =
        React.useState(true);
    const [isCreateSettingModalVisible, setCreateSettingModalVisible] =
    ...
    const loadSelectOptions = () => {
        let rolesPromise = tablesService.getTableRows(
            Schemas.Administration,
            "Roles"
        );
        Promise.all([rolesPromise]).then((data) => {
            let selectOptionsListUpdate = {};
            selectOptionsListUpdate["Role"] = data[0].Data.map((x) => {
    ...
    return (
        <React.Fragment>
            ...
            {loadingSettingColumns && <Loading></Loading>}
            ...
            {!loadingSettingColumns && users && users.length > 0 && (
                <div>
                    <ApiraiserTable headers={displayedColumns}>
                        {users &&
                            users.map((row, i) => (
                                <tr key={"row_" + (i + 1)}>
